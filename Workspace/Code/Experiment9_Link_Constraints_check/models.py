...
def LinkConstraints1(embedding, link, weight_decay=0.1):
    # Link Constraints
    batch_size = len(embedding)
    Loss = []
    for i in range(batch_size-1):
        for j in range(i+1, batch_size):
            if link[i] == link[j]:
                Loss.append((embedding[i] - embedding[j]).norm(2))
    Loss = torch.stack(Loss).sum()
    Loss = Loss * 0.5 * weight_decay
    return Loss

def LinkConstraints2(embedding, link, weight_decay=0.1):
    # Link Constraints
    batch_size = len(embedding)
    Loss = []
    for i in range(batch_size-1):
...
if __name__ == '__main__':
    ...
    b = LinkConstraints1(e, link)
    b = LinkConstraints2(e, link)
