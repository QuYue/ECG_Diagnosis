...
import drawing
#%% Input Arguments
parser = argparse.ArgumentParser(description='Experiment7(CNN_Transformer): Train the model for diagnosing the heart disease by the ECG.')
    ...
    for epoch in range(Args.epoch):
        ...
        for step, (x, y) in enumerate(loader_train):  # input batch data from train loader
            ...
            for i in x:
                ...
                output.append(cnn_transformer(input))
        ...
        F1.append(f1_test)
        if Args.show_plot:
            drawing.draw_result([Accuracy, F1], fig, ['CNN_Transformer Accuracy', 'CNN_Transformer F1'], True)
        del x,input,output
