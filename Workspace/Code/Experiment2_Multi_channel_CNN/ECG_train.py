...
import drawing
#%% Input Arguments
parser = argparse.ArgumentParser(description='Experiment2(Multi_channel_CNN): Train the model for diagnosing the heart disease by the ECG.')
...
if __name__ == '__main__':
    ...
    #%% ########## Create model ##########
    print('>>>>> Create model')
    mc_cnn = models.MC_CNN().cuda() if Args.cuda else models.MC_CNN()
    # optimizer
    optimizer = torch.optim.Adam(mc_cnn.parameters(), lr=Args.learn_rate)
    # loss function
